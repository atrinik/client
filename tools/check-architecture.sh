...
jq -e '
  ...
  (deps("atrinik-protocol-adapter") | sort == ["atrinik-actions", "atrinik-session"]) and
  (deps("atrinik-platform") | sort == ["atrinik-actions", "sdl3"]) and
  ([.packages[] | select(.name | startswith("atrinik-")) | .dependencies[] | select(.source != null) | .name] | all(. == "sdl3" or . == "sha2"))
' "${metadata}" >/dev/null
