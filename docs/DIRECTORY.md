...
an unexpected encoding, invalid content type or length, malformed or changed
ETag, invalid Last-Modified publication time, schema error, identity error, or
freshness error rejects the whole response without replacing the
last-known-good record.

Each representation's strong `ETag` is an opaque, quoted, 3–128-byte origin
validator. The client never infers a hash algorithm or body digest from it.
`Last-Modified` is the exact alias-object publication time and must be at or
after the body's `generatedAt`, before `expiresAt`, and no more than the
protocol clock-skew allowance ahead of the client. A 304 must match the
requested ETag byte-for-byte; an included Last-Modified must match the cached
alias publication time. Revalidation may only refresh the local received-at
time while the body remains fresh. `Retry-After` is a canonical delta or HTTP
date bounded to 24 hours.
...
Only the public canonical body, its independently computed SHA-256, opaque
strong ETag, alias publication second, and local received-at second are cached
under the platform `directory` cache class. V2 records are bounded,
create-only, synchronized before atomic publication, and newest-first. A
verified legacy V1 record remains usable and is rewritten as V2 after a
successful conditional revalidation. Four candidates permit recovery from a
corrupt newest record; invalid records are reported and skipped. Cache
corruption never supplies a conditional ETag.
